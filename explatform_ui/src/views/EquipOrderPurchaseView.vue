        ...
        <table class="equip-table">
          ...
          <tbody>
            ...
            <tr
              ...
              :key="item.id"
              :class="{ 'editing-row': editingOrderId === item.id }"
            >
              <td class="checkbox-col">
                <input
                  ...
                  :disabled="isOrderEditing && editingOrderId !== item.id"
                />
              </td>
            ...
            <tr v-if="!isOrderAdding && paginatedOrderItems.length === 0">
              <td colspan="8" class="no-data">No order data found</td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <table class="equip-table">
          ...
          <tbody>
            ...
            <tr v-if="!isPurchaseAdding && paginatedPurchaseItems.length === 0">
              <td colspan="5" class="no-data">No purchase data found</td>
            </tr>
          </tbody>
        </table>
      </div>
...
export default {
  ...
  computed: {
    ...
    isAllCurrentOrderPageSelected() {
      ...
      return this.paginatedOrderItems.every((item) =>
        this.selectedOrderIds.includes(item.id)
      );
    },
  ...
  methods: {
    ...
    handleOrderSave() {
      if (this.isOrderAdding) {
        ...
        const newItem = {
          ...
          orderQty: this.orderEditForm.orderQty,
          getQty: this.orderEditForm.getQty,
          usingQty: this.orderEditForm.usingQty,
        };
    ...
    toggleSelectAllOrderCurrentPage(event) {
      ...
      const currentPageIds = this.paginatedOrderItems.map((item) => item.id);
    ...
    handlePurchaseSave() {
      if (this.isPurchaseAdding) {
        ...
        const newItem = {
          ...
          purchaseQty: this.purchaseEditForm.purchaseQty,
          deliverQuantity: this.purchaseEditForm.deliverQuantity,
        };
